...
// ─── Tuning ──────────────────────────────────────────────────────────────────
// Reduced FPS/quality/size so JPEG stays below ESP32 MAX_JPEG_FRAME_BYTES = 8000
const TARGET_FPS      = 3
  ...
  _inject(`
    (function() {
      try {
        ...
        if (
          typeof userLat !== 'undefined' &&
          userLat !== null &&
          typeof userLng !== 'undefined' &&
          userLng !== null &&
...
async function _processFrame(base64) {
  ...
  if (jpegBytes.length > MAX_JPEG_BYTES) {
    console.warn(`[MapStreamer] JPEG too large: ${jpegBytes.length}B — skipped`)
    return
  }
